...
| **AI Services** | Claude API (via MCP); optional **local** file-transcription STT via **`mcp-transcribe`** MCP (installed via `/career-navigator:setup-transcribe`); prep/mock interviews are text-only |
...
**├── .mcp.json** — may include Anthropic **HTTP MCP** entries for **Gmail** / **Google Calendar** and/or **`ms365`** (Outlook / Microsoft 365) as an alternate channel (no secrets); other MCPs (e.g. Apify for salary) are configured in the host app. **Does not** include **`mcp-transcribe`** — see below.

**├── mcp-transcribe/** — local MCP server (source-installed via `/career-navigator:setup-transcribe`, no prebuilt bundle) for recorded-interview file transcription; not published via GitHub Releases
...
| **/career-navigator:interview-capture** | Command | Opt-in **skill** (not an agent): transcribe a **recorded audio file** of the user's own interview, extract takeaways, update the application record as a multi-file transaction (detail file **`applications/<application_id>.json`** plus the **`tracker.json`** summary row); employer warning once; §13.1 retention. Uses **`mcp-transcribe`** (`/career-navigator:setup-transcribe` to install) when available. |
...
| **/career-navigator:linkedin-post-analytics** | Command | **Read-only** snapshot of the user’s **own** LinkedIn post analytics into **`networking.json`** `networking[]` (per-post **`analytics_history`**). Requires host **browser control** (e.g. **Claude in Chrome** MCP or **computer / browser use**) and **explicit user approval** before navigation; see **`linkedin-post-analytics`** skill. |
...
| **mock-interview** | Skill | Mock session: **guided** / **random** / **adaptive**; **stage** + **vibe**; **selects defaults** when mode/vibe omitted (see skill §2.1). Invokes **`interview-coach`** (`mock` mode). Text-only. Also invocable via `/career-navigator:mock-interview`. |
| **interview-capture** | Skill | **Not an agent.** Opt-in post-interview transcription of a **recorded audio file** (via **`mcp-transcribe`** or compatible STT), structured takeaways, application-record updates (detail file **`applications/<application_id>.json`** plus the **`tracker.json`** summary row); employer warning once; §13.1 retention. Also invocable via `/career-navigator:interview-capture`. |
...
| **setup-schedule** | Skill | **Phase 3.** Run once after launch to create the `career-navigator-daily-brief` Cowork scheduled task, and optionally a second `career-navigator-pipeline-refresh` task. Builds fully self-contained task prompts (resolved `{user_dir}` paths). The daily brief runs the daily-schedule workflow on the user's chosen cadence (default 7 AM daily): two-pass Gmail inbox scan (7-day catch-all + 30-day company backfill), follow-up alerting, and new job discovery appended to `recommendations.json`. The optional pipeline refresh runs the `pipeline-status` skill nightly (default 2 AM daily) to regenerate `pipeline-status.html`. Detects and updates existing tasks (either one independently) to prevent duplicates. Invocable via `/career-navigator:setup-schedule`. |
| **pipeline-status** | Skill | Generates the pipeline status page — a static, self-contained HTML page at `{user_dir}/CareerNavigator/pipeline-status.html`, with a co-located `pipeline-data.js` and an embedded `timeline.html` — from `tracker.json`, `recommendations.json`, and `passed.json`. Filterable by status / section / rating, with active / focus / closed / passed sections and an application timeline iframe. Always a fresh snapshot on request, and can be scheduled to regenerate nightly via `setup-schedule`. Opens in the browser automatically. Invocable via `/career-navigator:pipeline-status`. |
...
*Specification:* this section. *Closest shipped behavior:* `skills/pipeline-status/SKILL.md` for pipeline/dashboard flows using local data.
...
| **mcp-transcribe** (local MCP server) | Source-installed, no bundle | **Local** file transcription for **`interview-capture`** (recorded audio only — `prep-interview` and `mock-interview` are text-only). Install via `/career-navigator:setup-transcribe`, or manually per `mcp-transcribe/README.md`. Source: **`mcp-transcribe/`**; tools **`transcribe_file`**, **`backend_info`**. Not declared in project **`.mcp.json`**. |
| **Meetup / Eventbrite / Luma** | MCP + host browser/manual fallback | Event discovery for networking radar. Luma is optional and not shipped — use a Luma MCP already in the user's session (e.g. the independent **`alx1p/luma-mcp`** project). For Meetup/Eventbrite, use optional **Claude in Chrome**, **computer use**, or **manual copy/paste** fallback. |
...
*Data:* `{user_dir}/CareerNavigator/networking.json` → `networking[]` and `recruiter_relationships[]`. *Capture behavior:* `skills/linkedin-post-analytics/SKILL.md`.

`{user_dir}/CareerNavigator/networking.json` is a dedicated file (separate from `tracker.json`) with two top-level arrays. The **`networking`** array holds relationship and visibility artifacts—LinkedIn post analytics entries with **`type: "linkedin_post"`**, a stable **`url`** (LinkedIn activity URN URL), optional **`description`** / **`date_posted`**, and **`analytics_history`**: an array of dated objects capturing impressions, reach, reactions, comments, reposts, saves, sends, profile viewers attributed to the post, followers gained, link visits, optional **`links`**, and optional **`top_audience`** (industry, seniority, company size) when the UI exposes them. The **`recruiter_relationships`** array holds recruiter and agency contacts with their stage history, active mandates, and follow-up dates. **`networking-strategist`** reads both arrays for relationship context; schema details for `networking[]` are defined in **`skills/linkedin-post-analytics/SKILL.md`**.
...
* **Skills introduced / extended:** `interview-capture` (**skill**, not an agent — recorded-audio-file transcription via **`mcp-transcribe`** or compatible STT, structured tracker population — opt-in only; MVP scope: user's own recording only)
...
* **Event intelligence sources** for `event-radar` / `event-intelligence`: Luma via an optional, not-shipped MCP server if one is present in the user's session, with Meetup/Eventbrite handled as optional **Claude in Chrome**, **computer use**, or **manual copy/paste** fallback paths.
...
| **/career-navigator:linkedin-post-analytics** | Read-only own LinkedIn post metrics → **`networking.json`** `networking[]` (browser automation + user consent) |
