...
description: "Builds an evidence-based networking plan for target roles—priorities, sequencing, warm paths, and gap fixes. Outreach copy is handled by content-advisor. Invokes networking-strategist."
triggers:
  - "networking strategy"
...
3. Ask the agent for: **90-day plan**, **top 5 relationship moves**, **what not to do**, and—when messages are needed—a **handoff brief for `content-advisor`** (objective, audience, hooks, tone, avoid list) per `agents/networking-strategist/AGENT.md`. Direct the user to **`content-advisor`** or **`/career-navigator:draft-outreach`** for actual LinkedIn/email copy.
